...
import { Chart as ChartJS, CategoryScale, LinearScale, BarElement, Title, Tooltip, Legend, ArcElement, PointElement, LineElement } from 'chart.js'; // Thêm PointElement, LineElement
...
export default function Home() {
  ...
  useEffect(() => {
    axios.get(API_BASE_URL).catch(err => console.log("Server wake-up call failed, might be starting up."));
  ...
  const handleAnalysis = async (suggestion) => {
    setIsLoading(true);
    const groupByCol = columns.find(c => c !== suggestion.column && !c.includes('date'));

    try {
        const res = await axios.post(`${API_BASE_URL}/api/datasets/${selectedDataset}/analyze`, {
            type: suggestion.type,
            column: suggestion.column,
            group_by_col: suggestion.type.startsWith('resample') ? 'N/A' : groupByCol,
        });
        ...
        <div className="lg:col-span-2 bg-gray-800/50 rounded-xl p-8 border border-gray-700 min-h-[400px] flex items-center justify-center">
          ...
          {chartData && (
            <div className="w-full">
              ...
              {/* Thêm logic render cho biểu đồ đường */}
              ...
              {chartData.type === 'line' && <Line options={{ responsive: true }} data={chartData.data} />}
            </div>
          )}
        </div>
      </div>
